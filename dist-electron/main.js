...
const Readable$1 = require$$2$1.Readable;
...
const PromiseConnection2 = connection$1;
...
let Connection$2 = class Connection extends BaseConnection2 {
  promise(promiseImpl) {
    const PromiseConnection3 = connection$1;
...
(function(exports) {
  ...
  exports.PromisePool = PromisePool2;
  exports.PromiseConnection = PromiseConnection3;
  exports.PromisePoolConnection = PromisePoolConnection3;
...
const getProducts = async (query2 = "") => {
  const connection2 = await getConnection();
  let sql = "SELECT * FROM products";
  const params = [];
  if (query2) {
    sql += " WHERE name LIKE ? OR bar_code LIKE ?";
    const likeQuery = `${query2}%`;
    params.push(likeQuery, likeQuery);
  }
  sql += " ORDER BY created_at DESC";
  const [rows] = await connection2.execute(sql, params);
  connection2.end();
  return rows;
};
const getProductByNameOrBarCode = async (query2) => {
  const connection2 = await getConnection();
  let sql = "SELECT * FROM products WHERE name = ? OR bar_code = ?  ";
  const params = [];
  params.push(query2, query2);
  const [rows] = await connection2.execute(sql, params);
  if (rows.length > 0) {
    connection2.end();
    return rows;
    ...
    const paramsLike = [];
    let sqlLike = "SELECT * FROM products WHERE name LIKE ? OR bar_code LIKE ? LIMIT 1";
    paramsLike.push(`${query2}%`, `${query2}%`);
...
ipcMain.handle("add-product", async (_, product) => {
  try {
    ...
    const userError = error.message.includes("ER_DUP_ENTRY") ? "Un produit avec ce nom ou ce code-barres existe déjà." : "Échec de l'ajout du produit. Veuillez vérifier les informations.";
    return { success: false, error: userError };
  }
});
ipcMain.handle("get-products", async (_, query2) => {
  try {
    const products = await getProducts(query2);
...
ipcMain.handle("get-product-by-name-or-bar-code", async (_, query2) => {
  try {
    const products = await getProductByNameOrBarCode(query2);
    return { success: true, data: products };
  } catch (error) {
...
ipcMain.handle("update-product", async (_, product) => {
  try {
    ...
    const userError = error.message.includes("ER_DUP_ENTRY") ? "Un produit avec ce nom ou ce code-barres existe déjà." : "Échec de la mise à jour du produit. Veuillez vérifier les informations.";
    return { success: false, error: userError };
  }
});
ipcMain.handle("delete-product", async (_, id) => {
  try {
    ...
    const userError = error.message.includes("ER_ROW_IS_REFERENCED") ? "Impossible de supprimer ce produit car il est lié à d'autres données." : "Échec de la suppression du produit. Veuillez réessayer.";
    return { success: false, error: userError };
  }
});
